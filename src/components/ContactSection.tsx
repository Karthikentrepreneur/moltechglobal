...
const ContactSection = () => {
  ...
  const activeOffice = useMemo(() => OFFICES.find(o => o.country.toLowerCase() === selected.toLowerCase()), [selected]);
  return <section className="relative">
      {/* Hero: non-copyright 3D globe image */}
      <div className="relative h-[46vh] min-h-[320px] w-full overflow-hidden">
        <img src="/images/3d-globe.jpg" /* place a royalty-free 3D globe image here */ alt="3D global network backdrop" className="h-full w-full object-cover" loading="eager" decoding="async" />
      ...
      <div className="section-padding bg-muted/30">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-10">
          ...
          <Card className="bg-royal-gradient text-white border border-royal-blue/20 lg:col-span-1 rounded-2xl">
            <CardHeader className="pb-2">
              <CardTitle className="text-white">Countries</CardTitle>
            </CardHeader>
            <CardContent className="bg-gray-950 bg-[bg-royal-gradient]">
              <ul className="space-y-3">
                {OFFICES.map(o => {
                const isActive = o.country === selected;
                return <li key={o.country}>
                      <button onClick={() => setSelected(o.country)} className={["w-full text-left px-5 py-3", "uppercase tracking-wide font-semibold", "rounded-full border transition-all",
                  // ✨ CTA-like pill styling
                  isActive ? "bg-royal-blue text-white border-royal-blue shadow-sm" : "bg-white text-royal-blue border-royal-blue/40 hover:bg-white/90 hover:shadow"].join(" ")} aria-current={isActive ? "true" : "false"}>
                        {o.country}
                      </button>
                    </li>;
              })}
              ...
              <CardContent className="space-y-4">
                ...
                <div className="mt-4">
                  {activeOffice?.mapUrl && activeOffice.mapUrl !== "#" ? <div className="aspect-video w-full overflow-hidden rounded-2xl border border-border">
                    ...
                    </div> : <div className="aspect-video w-full rounded-2xl border border-dashed border-border grid place-items-center bg-white/50">
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  ...
                  <Button variant="outline" className="btn-ghost" onClick={() => window.scrollTo({
                  top: 0,
                  behavior: "smooth"
                })}>
                    Back to Top
